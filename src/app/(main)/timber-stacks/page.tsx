// frontend/src/app/(main)/timber-stacks/page.tsx
'use client'; 
...
export default function TimberStacksPage() {
    ...
    const handleLocationChange = async (id: number, newLocation: { latitude: number, longitude: number }) => {
        ...
        try {
            ...
            if (updatedStack) {
                const successMessage = `${updatedStack.clientName} - ${updatedStack.name} location updated.`;
                setSnackbar({ open: true, message: successMessage, severity: 'success' });
            }
            setMarkerToEnableMove(null);
        } catch (err: any) {
            setSnackbar({ open: true, message: err.message || `Failed to update location. Reverting.`, severity: 'error' });
    ...
    const handleMapClick = (coords: [number, number]) => {
        if (!canCreate) { setSnackbar({ open: true, message: "No permission to add markers.", severity: 'info' }); return; }
    ...
    const handleSaveOtherMarker = async (data: ICreateOtherMarkerDto | IUpdateOtherMarkerDto, id?: number) => {
        ...
        try {
            if (id) {
                ...
                setSnackbar({ open: true, message: 'Other marker updated successfully.', severity: 'success' });
            } else {
                ...
                setSnackbar({ open: true, message: 'Other marker created successfully.', severity: 'success' });
            }
            handleCloseModals(true); // Close and reload
        } catch (err: any) { setModalError(err.response?.data?.message || "Failed to save."); } 
    ...
    const handleDoubleClick = (marker: IMapTimberStack) => {
        if (markerToEnableMove !== null) {
            setSnackbar({ open: true, message: `Another marker is already unlocked.`, severity: 'info' });
            return;
        }
    ...
    const confirmMove = () => {
        if (moveConfirmation) {
            ...
            setSnackbar({ open: true, message: `Marker "${moveConfirmation.name}" is now unlocked.`, severity: 'info' });
        }
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setSnackbar({ open: true, message: `${type} '${item.name}' deleted successfully.`, severity: 'success' });
            setDeleteConfirmation(null);
            await reloadData();
        } catch (err: any) {
            const errorMsg = err.response?.data?.message || err.message || `Failed to delete ${type}.`;
            if (errorMsg.includes("is still referenced")) {
                setSnackbar({ open: true, message: `Cannot delete "${item.name}". It is currently in use.`, severity: 'error' });
            } else {
    ...
    if (!canView) return <Paper sx={{ p: 3, m: 2 }}><Alert severity="error">You do not have permission to view the map.</Alert></Paper>;

     return (
         <Box sx={{ height: '100%', width: '100%', position: 'relative', overflow: 'hidden' }}>
            <Box sx={{ position: 'absolute', top: 10, left: 52, right: 16, zIndex: 1000 }}>
                <Paper sx={{p: 2, backgroundColor: 'rgba(255, 255, 255, 0.25)', backdropFilter: 'blur(2px)', borderRadius: 2}}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <ForestIcon color="primary" />
                            <Typography variant="h5" sx={{ fontWeight: 'bold' }}>Puulaani Map View</Typography>
                        </Box>
                        <Chip label={isConnected ? 'Real-time Connected' : 'Connecting...'} color={isConnected ? 'success' : 'warning'} size="small" variant="outlined" />
                    </Box>
    ...
    {/* --- THIS IS THE NEW TOPOGRAPHY/TERRAIN LAYER --- */}
    <LayersControl.BaseLayer name="Topographic">
        ...
        />
                        ...
                        <LayersControl.Overlay checked name="Timber Stacks">
                            <FeatureGroup>
                                {(timberStacks || []).map(stack => {
                                     const customer = (clientList || []).find(c => String(c.id) === String(stack.clientId)) || { id: '0', name: 'Unknown Client', clientId: '0', clientName: 'Unknown Client', targetColor: '#808080' };
                                    return <PuulaaniMarker key={`stack-${stack.id}`} marker={stack} customer={customer} isDraggable={markerToEnableMove === stack.id} onEdit={handleOpenEditModal} onDelete={(m) => setDeleteConfirmation({type:'Puulaani', item: m})} onLocationChange={handleLocationChange} onDoubleClick={handleDoubleClick}/>;
                                })}
                            </FeatureGroup>
                        </LayersControl.Overlay>

                        <LayersControl.Overlay checked name="Unloading Sites">
                            <FeatureGroup>
                                {(dropoffLocations || [])
                        ...
                        <LayersControl.Overlay checked name="Other Markers">
                            <FeatureGroup>
            ...
            <ConfirmationDialog open={!!deleteConfirmation} onClose={() => setDeleteConfirmation(null)} onConfirm={confirmDelete} title={`Delete ${deleteConfirmation?.type}`} message={`Are you sure you want to delete "${deleteConfirmation?.item.name}"?`} isConfirming={isSaving} />
            <ConfirmationDialog open={!!moveConfirmation} onClose={() => setMoveConfirmation(null)} onConfirm={confirmMove} title="Confirm Marker Move" message={`Are you sure you want to unlock "${moveConfirmation?.name}"?`} confirmButtonText="Yes, Unlock" />
