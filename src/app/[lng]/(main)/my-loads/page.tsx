...
export default function DriverDashboardPage() {
    ...
    const { t } = useTranslation(['myLoads']);
            ...
            return (
                 <Box sx={{ p: 3, height: '100%' }}>
                    <Button startIcon={<ArrowBackIcon />} onClick={() => setView('mode-select')}>
                        {t('buttons.backToModeSelection')}
                    </Button>
                    <Typography variant="h4" sx={{mt: 2}}>{t('titles.consignments')}</Typography>
                    <Paper sx={{p:4, mt: 2}}>
                        <Typography color="text.secondary">
                            Consignment list view will be implemented here. (this text is hardcoded)
                        </Typography>
                    </Paper>
                </Box>
            );
