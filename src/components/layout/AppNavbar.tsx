...
export default function AppNavbar() {
    ...
    const handleCategoryMenuOpen = (event: React.MouseEvent<HTMLElement>, menuName: string) => {
    ...
    const getFilteredNavItems = (items: NavItemConfig[]): NavItemConfig[] => {
        ...
        return items.map(item => {
            if (item.children) {
                const visibleChildren = getFilteredNavItems(item.children);
    ...
    const topNavLinks = getFilteredNavItems(navigationItems).filter(item => item.isTopNav === true);
        ...
        >
            <Toolbar>
                            ...
                            if (item.children) {
                                return (
                                    <Box key={item.text}>
                                        <Button
                                            startIcon={
                                                <Box component="span" sx={{ display: 'inline-flex', fontSize: '1.25rem' }}>
                                                    {item.icon}
                                                </Box>
                                            }
                                            onClick={(e) => handleCategoryMenuOpen(e, item.text)}
                                        ...
                                        >
                                            {item.children.map((child) => {
                                                const isChildActive = !!(child.path && pathname.startsWith(child.path));
                                                return (
                                                    <MenuItem
                                                        key={child.path}
                                                        component={Link}
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Tooltip title={`Toggle ${themeMode === 'light' ? 'Dark' : 'Light'} Theme`}>
                        <IconButton sx={{ ml: 1 }} onClick={toggleThemeMode} color="inherit">
                            {themeMode === 'dark' ? <LightModeIcon /> : <Brightness4Icon />}
                        </IconButton>
                    </Tooltip>
                    
                    <Tooltip title={`Switch to ${navLayout === 'left' ? 'Top' : 'Left'} Navigation`}>
                        ...
                        <Box sx={{ display: { xs: 'flex', md: 'none' } }}>
                            ...
                            <Menu anchorEl={anchorElNav} open={Boolean(anchorElNav)} onClose={handleCloseNavMenu}>
                                {topNavLinks.map((item) => {
                                    ...
                                    return [
                                        <MenuItem key={item.text} disabled sx={{ opacity: '1 !important', mt: 1 }}>
