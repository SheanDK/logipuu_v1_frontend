    ...
    return (
        <Dialog open={open} onClose={onCloseAction} fullWidth maxWidth={isEditMode ? "sm" : "md"} TransitionComponent={Fade} PaperProps={{ sx: { background: 'linear-gradient(to top, #eef1f5 0%, #ffffff 100%)', borderRadius: 4, boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.37)'}}}>
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', bgcolor: '#37474f', color: 'white', py: 2 }}>
                ...
                <Box>
                    <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>{isEditMode ? 'Edit Load' : 'Create New Load'}</Typography>
                    {!isEditMode && <Typography variant="caption" sx={{ display: 'block', opacity: 0.8 }}>From: {puulaaniDetails?.puulaani.nimi}</Typography>}
                </Box>
                ...
                <FormProvider {...legMethods}>
                    <Box component="form" id="leg-form" onSubmit={isEditMode ? handleLegSubmit(handleUpdateSubmit, onFormError) : handleLegSubmit(handleAddLeg, onFormError)}>
                        <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #f5f7fa 0%, #ffffff 100%)', border: '1px solid #e0e0e0', borderRadius: 2 }}>
                            <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2.5, color: '#1565c0', display: 'flex', alignItems: 'center', gap: 1 }}>{isEditMode ? '📝 Load Details' : '➕ Add Load Leg'}</Typography>
                            <Grid container spacing={2.5} alignItems="center">
                                <Grid item xs={12} sm={4}><Controller name="puutavaraId" control={legControl} rules={{ required: 'Required' }} render={({ field }) => (<FormControl sx={{ width: 300 }} fullWidth error={!field.value} disabled={isEditMode}><InputLabel>Timber Type *</InputLabel><Select {...field} label="Timber Type *">{availableTasks.map(task => (<MenuItem key={task.puutavaraId} value={String(task.puutavaraId)}>{task.laji}</MenuItem>))}</Select></FormControl>)}/></Grid>
                                <Grid item xs={12} sm={4}><TextField label="Drop-off Location" value={selectedTask?.purkupaikkaName || ''} fullWidth disabled variant="filled" /></Grid>
                                <Grid item xs={12} sm={4}><Controller name="receptionNo" control={legControl} render={({ field }) => <TextField {...field} label="Reception Number" fullWidth />}/></Grid>
                                <Grid item xs={12} sm={4}><TextField label="Total (m³)" value={selectedTask ? Number(selectedTask.kuutiot).toFixed(2) : ''} fullWidth disabled variant="filled" /></Grid>
                                <Grid item xs={12} sm={4}><Controller name="volume" control={legControl} rules={{ required: 'Required' }} render={({ field, fieldState }) => <TextField  {...field} label="Hauled (m³)" type="number"  fullWidth error={!!fieldState.error} helperText={fieldState.error?.message} />}/></Grid>
                                <Grid item xs={12} sm={4}><TextField label="Remaining (m³)" value={calculatedRemaining} fullWidth disabled variant="filled" /></Grid>
                                <Grid item xs={12} sm={4}><Controller name="route" control={legControl} render={({ field }) => <TextField {...field} label="Route" fullWidth />}/></Grid>
                                <Grid item xs={12} sm={4}><Controller name="km" control={legControl} render={({ field }) => <TextField {...field} label="Freight (km)" type="number" fullWidth />}/></Grid>
                                <Grid item xs={12} sm={4}><Controller name="notes" control={legControl} render={({ field }) => <TextField {...field} label="Additional Notes" fullWidth />}/></Grid>
                            </Grid>
                            {!isEditMode && <Button type="submit" startIcon={<AddCircleIcon />} variant="contained" sx={{ mt: 3, width: '100%', py: 1.5, backgroundColor: '#607d8b', '&:hover': {backgroundColor: '#546e7a'} }}>Add to Trip</Button>}
                        </Paper>
                    </Box>
                </FormProvider>
                {!isEditMode && (<>
                    <Divider sx={{ my: 2 }}><Chip label="Trip Legs" icon={<LocalShippingIcon fontSize="small" />} /></Divider>
                    <TableContainer component={Paper} elevation={0} variant='outlined' sx={{ borderRadius: 2 }}><Table size="small"><TableHead><TableRow sx={{bgcolor: 'action.hover'}}><TableCell>Timber</TableCell><TableCell align="right">Volume (m³)</TableCell><TableCell>Reception No.</TableCell><TableCell align="center">Actions</TableCell></TableRow></TableHead><TableBody>{(fields as any[]).length > 0 ? (fields as any[]).map((field, index) => (<TableRow key={field.id} hover sx={{ '&:last-child td': { borderBottom: 0 } }}><TableCell sx={{ fontWeight: 500 }}>{field.taskDetails.laji}</TableCell><TableCell align="right" sx={{ fontWeight: 600, color: '#1976d2' }}>{field.volume}</TableCell><TableCell>{field.receptionNo || '—'}</TableCell><TableCell align="center"><IconButton size="small" color="error" onClick={() => remove(index)}><DeleteIcon fontSize="small" /></IconButton></TableCell></TableRow>)) : (<TableRow><TableCell colSpan={4} align="center" sx={{ py: 6, color: 'text.secondary', fontStyle: 'italic' }}><Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}><Typography variant="body2" color="text.secondary">📦 No legs added yet</Typography><Typography variant="caption" color="text.disabled">Add your first load leg using the form above</Typography></Box></TableCell></TableRow>)}</TableBody></Table></TableContainer>
                </>)}
            </DialogContent>
            <DialogActions sx={{ p: 2, borderTop: '1px solid #e0e0e0', backgroundColor: '#f4f6f8' }}>
                <Button onClick={onCloseAction} variant="text" color="secondary">Cancel</Button>
                {isEditMode ? (<Button variant="contained" color="primary" startIcon={<SaveIcon />} onClick={handleLegSubmit(handleUpdateSubmit)}>Save Changes</Button>) : (<Button variant="contained" color="success" startIcon={<SendIcon />} onClick={handleMainSubmit(handleFinalCreateSubmit)} disabled={fields.length === 0}>Confirm & Create Trip</Button>)}
            </DialogActions>
        </Dialog>
    );
}