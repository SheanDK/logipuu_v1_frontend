                ...
                <Stack spacing={3}>
                    ...
                    <Box>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1, px: 1 }}>
                            ...
                            <Button variant="contained" sx={{ backgroundColor: '#607d8b', '&:hover': { backgroundColor: '#546e7a' } }} size="small" startIcon={<AddCircleOutlineIcon />} onClick={() => onCreateLoadAction(editableDetails)} disabled={!editableDetails.puulaani}>{t('loads.create')}</Button>
                        </Box>
                        <TableContainer component={Paper} variant="outlined">
                            <Table size="small">
                                <TableHead>
                                                ...
                                                <StyledTableCell align="center">
                                                    {isActive ? (
                                                        ...
                                                        <Stack direction="row" spacing={0.5} justifyContent="center">
                                                            {canStart && (
                                                                <IconButton size="small" title={t('loads.actions.start')} color="success" onClick={(e) => { e.stopPropagation(); onStartTripAction(load); }}>
                                                                    <PlayCircleOutlineIcon />
                                                                </IconButton>
                                                            )}
                                                            {canEditOrDelete && (
                                                                <IconButton size="small" title={t('loads.actions.edit')}>
                                                                    <EditIcon fontSize="small" />
                                                                </IconButton>
                                                            )}
                                                            {canEditOrDelete && (
                                                                <IconButton size="small" color="error" title={t('loads.actions.delete')} onClick={(e) => { e.stopPropagation(); onDeleteLoadAction(load); }}>
