...
import {
    ...
    Button, Box, Typography, TextField, Stack, Divider
} from '@mui/material';
...
const ModifyLoadModal = ({ open, loadData, onClose, onSave, onDelete }: any) => {
    ...
    return (
        <Dialog
            ...
            fullWidth
            maxWidth="xs"
            PaperProps={{ sx: { borderRadius: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.12)' } }}
        >
            <DialogTitle component="div" sx={{ p: 2.5, display: 'flex', alignItems: 'center', gap: 1.5, bgcolor: '#fdfaf5', borderBottom: '1px solid #eee' }}>
                <LocalShippingIcon sx={{ color: '#a38f6d' }} />
            ...
            <DialogContent sx={{ p: 3 }}>
                <Stack spacing={3} sx={{ mt: 1 }}>
                    <Box sx={{ p: 2, borderRadius: '8px', bgcolor: '#fafafa', border: '1px dashed #d1d1d1', position: 'relative' }}>
                        <Typography variant="caption" sx={{ position: 'absolute', top: -10, left: 10, bgcolor: '#fafafa', px: 1, color: '#a38f6d', fontWeight: 'bold', fontSize: '10px', textTransform: 'uppercase' }}>
                            {t('chip-management:loadDetails.vehicleAndItem')}
                        </Typography>
                        <Typography variant="body1" sx={{ fontWeight: 700, color: '#333' }}>
                            {loadData?.rekNro} | {loadData?.abbreviation || loadData?.titleName || 'N/A'}
                        </Typography>
                    </Box>
            ...
            <DialogActions sx={{ p: 2, justifyContent: 'space-between', bgcolor: '#fbfbfb' }}>
