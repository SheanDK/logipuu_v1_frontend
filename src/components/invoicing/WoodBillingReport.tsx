...
const WoodBillingReport: React.FC<Props> = ({ rows }) => {
  ...
  const formatDate = (dateStr: string | null) => { if (!dateStr) return '-'; return dayjs(dateStr).format('DD.MM.YYYY'); };
  ...
  const handleDownloadPdf = () => {
    ...
    function generatePdfContent(doc: jsPDF) {
        ...
        doc.setFontSize(10); doc.setTextColor(51, 51, 51); doc.text("Total Value", totalValX, yPos + 12, { align: 'right' });
        doc.setFontSize(14); doc.setTextColor(51, 51, 51); doc.text(`${fix2(grandTotals.total)} €`, totalValX, yPos + 19, { align: 'right' });
            ...
            <Paper elevation={3} sx={{ p: { xs: 2, md: 4 }, bgcolor: 'background.paper' }} className="break-avoid">
                ...
                <Box className="grand-total-box" sx={{ mt: 4, p: 2, bgcolor: grandTotalBgColor, borderRadius: 1, border: `1px solid ${isDark ? '#444' : '#ccc'}`, pageBreakInside: 'avoid', '@media print': { border: '1px solid #000', backgroundColor: 'transparent' } }}>
                     ...
                     <Stack direction="row" spacing={3} sx={{ flexWrap: 'wrap', rowGap: 2 }}>
                        ...
                        <Box sx={{ ml: 'auto !important' }}><Typography variant="caption" color="primary" sx={{ '@media print': { color: '#000' } }}>{t('totalValue', {defaultValue: 'Total Value'})}</Typography><Typography variant="h5" fontWeight="bold" color="primary" sx={{ '@media print': { color: '#000' } }}>{fix2(grandTotals.total)} €</Typography></Box>
                     </Stack>
                </Box>
            </Paper>
        </Container>
